...
function applyRoom(room) {
  ...
  if (room.status==='lobby') { if($('#winnerDialog').open)$('#winnerDialog').close(); renderPlayers(room); renderHostPanel(); showScreen('lobbyScreen'); }
...
function renderHostPanel() { $('.host-panel h3').textContent=state.isHost?'Du bist Gastgeber':'Warte auf den Gastgeber'; $('.host-panel p').textContent=state.isHost?'Du startest die Mission – danach übernimmt der Autopilot.':'Sobald der Gastgeber startet, erscheint hier deine Karte.'; $('#startGameBtn').hidden=!state.isHost; $('#startGameBtn').disabled=!state.isHost; $('.interval-setting').hidden=!state.isHost; $('#drawInterval').disabled=!state.isHost; $('#drawInterval').value=String(state.room?.drawInterval||10); $('#newRoundBtn').hidden=false; $('#newRoundBtn').disabled=false; }
