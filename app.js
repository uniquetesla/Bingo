const $ = selector => selector.startsWith('#') ? document.getElementById(selector.slice(1)) : document.querySelector(selector);
const $$ = selector => selector.startsWith('#') ? [document.getElementById(selector.slice(1))].filter(Boolean) : [...document.querySelectorAll(selector)];
...
function applyRoom(room) {
  ...
  if (previousRoom?.code===room.code && (previousRoom.status==='playing'||room.status==='playing')) notifyPlayerChanges(previousRoom.players,room.players);
  ...
  if (room.status==='lobby' || room.returnedToLobby) { if($('#winnerDialog').open)$('#winnerDialog').close(); renderPlayers(room); renderHostPanel(); showScreen('lobbyScreen'); }
  else { renderBoard(); updateDrawDisplay(); renderGamePlayers(room); renderHostPanel(); showScreen('gameScreen'); if (room.status==='finished' && room.winnerId) showWinner(room); }
}
function notifyPlayerChanges(previousPlayers,players) { const previous=new Map(previousPlayers.map(player=>[player.id,player.name])); const current=new Map(players.map(player=>[player.id,player.name])); const joined=players.filter(player=>!previous.has(player.id)); const left=previousPlayers.filter(player=>!current.has(player.id)); const messages=[...joined.map(player=>`${player.name} ist der Lobby beigetreten.`),...left.map(player=>`${player.name} hat die Lobby verlassen.`)]; if(messages.length)toast(messages.join(' ')); }
...
function renderGamePlayers(room) { $('#gamePlayerCount').textContent=room.players.length; $('#gamePlayersList').innerHTML=room.players.map(player=>`<div class="game-player"><span class="avatar">${escapeHtml(initials(player.name))}</span><span>${escapeHtml(player.name)}${player.id===playerId?' (Du)':''}</span>${player.id===room.hostId?'<small>Gastgeber</small>':''}</div>`).join(''); }
function renderHostPanel() { const waiting=state.room?.status==='finished'; $('.host-panel h3').textContent=waiting?'Warte auf die Crew':state.isHost?'Du bist Gastgeber':'Warte auf den Gastgeber'; $('.host-panel p').textContent=waiting?'Die nächste Runde kann beginnen, sobald alle selbst zur Lobby zurückgekehrt sind.':state.isHost?'Du startest die Mission – danach übernimmt der Autopilot.':'Sobald der Gastgeber startet, erscheint hier deine Karte.'; $('#startGameBtn').hidden=!state.isHost||waiting; $('#startGameBtn').disabled=!state.isHost||waiting; $('.interval-setting').hidden=!state.isHost||waiting; $('#drawInterval').disabled=!state.isHost||waiting; $('#drawInterval').value=String(state.room?.drawInterval||10); $('#newRoundBtn').hidden=false; $('#newRoundBtn').disabled=false; }
...
function openRoomChoice() { const joining=state.mode==='join'; $('#choiceTitle').textContent=joining?'Raum beitreten':'Raum erstellen'; $('#choiceSubtitle').textContent=joining?'Gib den Code deines Gastgebers ein.':'Dein persönliches JL-Bingo ist gleich bereit.'; $('#joinForm').hidden=!joining; $('#createPanel').hidden=joining; showScreen('roomChoiceScreen'); if(joining)setTimeout(()=>$('#roomCode').focus(),50); }
