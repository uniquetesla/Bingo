...
function renderPlayers(room) { $('#playerCount').textContent=room.players.length; $('#playersList').innerHTML=room.players.map(player=>`<div class="player-row"><span class="avatar">${escapeHtml(initials(player.name))}</span><div><p>${escapeHtml(player.name)}${player.id===playerId?' (Du)':''}</p><small>${player.id===room.hostId?'Kommandant':'Crewmitglied'}</small></div><span class="status">● Bereit</span></div>`).join(''); }
function renderHostPanel() { $('.host-panel h3').textContent=state.isHost?'Du bist Gastgeber':'Warte auf den Gastgeber'; $('.host-panel p').textContent=state.isHost?'Du bestimmst, wann die Runde beginnt und ziehst die Zahlen.':'Sobald der Gastgeber startet, erscheint hier deine Karte.'; $('#startGameBtn').hidden=!state.isHost; $('#drawBtn').hidden=!state.isHost; $('#newRoundBtn').hidden=!state.isHost; }
...
function updateDrawDisplay() { const current=state.drawn.at(-1); $('#currentBall').textContent=current??'?'; $('#currentBall').classList.toggle('empty',!current); $('#drawnCount').textContent=state.drawn.length; $('#callStatus').textContent=current?`${current} wurde gezogen`:'Ziehe die erste Zahl!'; $('#recentBalls').innerHTML=state.drawn.length?state.drawn.slice(-8).reverse().map(n=>`<span>${n}</span>`).join(''):'<span class="muted">Noch keine</span>'; }
...
$('#startGameBtn').addEventListener('click',async()=>{const room=await perform('game:start');if(room)applyRoom(room)});
$('#drawBtn').addEventListener('click',async()=>{const room=await perform('game:draw');if(room)applyRoom(room)});
$('#bingoBtn').addEventListener('click',async()=>{const room=await perform('game:bingo');if(room)applyRoom(room)});
$('#newRoundBtn').addEventListener('click',async()=>{const room=await perform('game:new-round');if(room){$('#winnerDialog').close();applyRoom(room)}});
